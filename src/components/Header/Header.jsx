...
export function Header() {
    ...
    const deleteAccount = async () => {
        try {
            await api.delete(`/admin/${userData?.id}`)
        } catch (err) {
            toast.error('Tarmoq xatosi. Iltimos, internetga ulanishingizni tekshiring.', {
                style: {
                    width: '425px',
    ...
    return <header className="header w-full max-w-full sticky top-0 z-100">
                ...
                {open && (
                    <div className="absolute right-0 top-[calc(100%+8px)] w-[200px] bg-white rounded-[4px] shadow-[0px_0px_6px_0px_#00000021] border border-[#DBDFEA] z-40 overflow-hidden">
                        ...
                        <div className='border-t border-b border-[#DBDFEA]'>
                            ...
                            <button
                                className="w-full max-w-full flex items-center gap-[12px] px-[20px] py-[16px] hover:bg-[#f5f6fa] duration-[.2s]"
                        ...
                        <div>
                            <button
                                className="w-full flex items-center gap-[12px] px-[20px] py-[18.11px] hover:bg-[#fff5f5] duration-[.2s]"
                                onClick={() => setOpenDelAccountModal(true)}>
        ...
        {openModal && createPortal(
            <div
                id='change-password-modal-wrap'
                className='w-full max-w-full h-full fixed top-0 flex justify-center items-center z-100 bg-[#00000066]'
                onClick={(e) => { if (e.target.id === 'change-password-modal-wrap') setOpenModal(false) }}>
        ...
        {openDelAccountModal && createPortal(
            <div
                id='del-account-modal'
                className="w-full max-w-full h-full fixed top-0 flex justify-center items-center z-100 bg-[#00000066]"
                onClick={(e) => { if (e.target.id === 'del-account-modal') setOpenDelAccountModal(false) }}>
                <div className="modal-card w-[440px] p-[24px] flex flex-col items-center gap-[32px] rounded-[6px] bg-[#FFF]">
                    <div className="w-full max-w-full flex items-center justify-between gap-[14px]">
                        <h6 className='text-[#364A63] text-[20px] font-bold leading-[22px] tracking-[-0.2px]'>Tasdiqlash</h6>

                        <button className="exit-btn" onClick={() => setOpenDelAccountModal(false)}>
                            <img src={exit} alt="Exit" />
                        </button>
                    </div>

                    <p className='text-[#526484] text-[16px] leading-[23.1px] tracking-normal'>
                        Siz rostan ham akkauntdan chiqmoqchimisiz?
                    </p>

                    <div className="flex items-center gap-[20px]">
                        <button
                            className="p-[8px_26.5px] border border-[#DBDFEA] rounded-[4px] text-[#526484] text-[13px] font-bold leading-[20px] tracking-[0.26px] bg-[#FFF] duration-[.3s] hover:bg-[#f5f6fa]"
                            onClick={() => setOpenDelAccountModal(false)}>Bekor qilish</button>

                        <button
                            className="p-[8px_30.5px] rounded-[3px] text-[#FFF] text-[13px] leading-[20px] tracking-[0.26px] bg-[#E85347] duration-[.3s] hover:bg-[#df3024]"
                            onClick={() => {
                                deleteAccount()
                                navigate('/login')
                            }}>Ha, chiqish</button>
